...
def actualizar_graficas():
    ...
    # Definición de constantes para etiquetas
    ETIQUETA_TIEMPO = 'Tiempo (s)'
    ETIQUETA_AMPLITUD = 'Amplitud'
    ETIQUETA_FRECUENCIA = 'Frecuencia (Hz)'

    # Uso de constantes en la creación de gráficas
    axs[0, 0].plot(tiempo_sinusoidal, onda_sinusoidal, label='Sinusoidal')
    axs[0, 0].set_xlabel(ETIQUETA_TIEMPO)
    axs[0, 0].set_ylabel(ETIQUETA_AMPLITUD)
    axs[0, 0].legend()
    axs[0, 1].plot(frecuencias_sinusoidal, np.abs(fft_sinusoidal), label='Transformada')
    axs[0, 1].set_xlabel(ETIQUETA_FRECUENCIA)
    axs[0, 1].set_ylabel(ETIQUETA_AMPLITUD)
    axs[0, 1].legend()
    

    # Cuadrada
    axs[1, 0].plot(tiempo_cuadrada, onda_cuadrada, label='Cuadrada')
    axs[1, 0].set_xlabel(ETIQUETA_TIEMPO)
    axs[1, 0].set_ylabel(ETIQUETA_AMPLITUD)
    axs[1, 0].legend()
    axs[1, 1].plot(frecuencias_cuadrada, np.abs(fft_cuadrada), label='Transformada')
    axs[1, 1].set_xlabel(ETIQUETA_FRECUENCIA)
    axs[1, 1].set_ylabel(ETIQUETA_AMPLITUD)
    axs[1, 1].legend()

    # Triangular
    axs[2, 0].plot(tiempo_triangular, onda_triangular, label='Triangular')
    axs[2, 0].set_xlabel(ETIQUETA_TIEMPO)
    axs[2, 0].set_ylabel(ETIQUETA_AMPLITUD)
    axs[2, 0].legend()
    axs[2, 1].plot(frecuencias_triangular, np.abs(fft_triangular), label='Transformada')
    axs[2, 1].set_xlabel(ETIQUETA_FRECUENCIA)
    axs[2, 1].set_ylabel(ETIQUETA_AMPLITUD)
    axs[2, 1].legend()
